...
def train(args, optimizer, criterion, train_loader, model, validate_loader):
    ISOTIMEFORMAT = '%Y_%m_%d_%H_%M'
    theTime = datetime.datetime.now().strftime(ISOTIMEFORMAT)
    create_excel(theTime)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels, event_labels) in enumerate(train_loader):
            ...
            # 模型输出
            optimizer.zero_grad()
            class_outputs, domain_outputs, text_image = model(train_text, train_image, train_mask)
        ...
        for i, (validate_data, validate_labels, event_labels) in enumerate(validate_loader):
            ...
            validate_outputs, domain_outputs, text_image = model(validate_text, validate_image, validate_mask)
        ...
        excel_write(theTime, epoch+1, np.mean(cost_vector), np.mean(class_cost_vector), np.mean(domain_cost_vector), np.mean(acc_vector),validate_acc)
        ...
        if validate_acc > best_validate_acc:
            ...
            args.best_validate_dir = best_validate_dir

    torch.save(torch.load(args.best_validate_dir), "result/20.pkl")
...
def test(args, test_loader, model, W):
    ...
    for i, (test_data, test_labels, event_labels) in enumerate(test_loader):
        ...
        test_outputs, domain_outputs, text_image = model(test_text, test_image, test_mask)
        ...
        if i == 0:
            ...
            test_feature = Transform_Tensor_Numpy(text_image.squeeze())
        else:
            ...
            test_feature = np.concatenate((test_feature, Transform_Tensor_Numpy(text_image.squeeze())), axis=0)

    np.save('fusion_features.npy', test_feature)
    np.save('label_fusion_features.npy',test_pred)
